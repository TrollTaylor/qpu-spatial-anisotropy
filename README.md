...
## 3. Usage Guidelines
  ...
  ```bash
  python qpu_anisotropy_sweep.py
  ```
...
### hardware_anisotropy_sweep_data.csv
  >A raw, unweighted count ledger recording registration counts ($n_{00}, n_{11}, n_{01}, n_{10}$) alongside absolute laboratory orientation angles ($\phi$).
### spatial_anisotropy_residual_map.png
  >A comparative mapping plot displaying physical raw telemetry against the standard isotropic null hypothesis baseline and the anisotropic SME target model.
